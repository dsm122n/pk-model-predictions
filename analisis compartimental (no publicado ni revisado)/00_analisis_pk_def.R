...
summarise(parametros_pk_VC, mean)
# ordenando datos 
while(TRUE){
...
ggplot()+
  ...
  geom_boxplot(data = datos_vc, aes(x=t_min, y=concentration, group = t_min),size = 1, col = "#00000090")+
  geom_point(data = datos_vc, aes(x=t_min, y=concentration, color = patient),size = 2, alpha = 0.5)+
...
write.csv(todos_pk, "00 parametros PK todos.csv", row.names  = FALSE)
